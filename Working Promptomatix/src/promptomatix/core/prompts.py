...
def validate_synthetic_data(task, data, input_fields, output_fields):
    """
    Versione Universale e Agnostica.
    Si fida dei campi estratti ma gestisce le discrepanze tra dizionari e liste.
    """
    
    # 1. Estrazione INPUT: Solo ciò che è esplicitamente definito come input
    input_data = {k: v for k, v in data.items() if k in input_fields}
    
    # 2. Estrazione OUTPUT: Solo ciò che è esplicitamente definito come output
    output_data = {k: v for k, v in data.items() if k in output_fields}
    
    # 3. LOGICA DI FALLBACK AGNOSTICA (Niente nomi hardcoded)
    # Se input_data è vuoto, prendiamo la prima chiave del dizionario (perché un input deve esserci)
    if not input_data and data:
        first_key = list(data.keys())[0]
        input_data = {first_key: data[first_key]}
        
    # Se output_data è vuoto, prendiamo tutto quello che non è nell'input
    if not output_data:
        input_keys = set(input_data.keys())
        output_data = {k: v for k, v in data.items() if k not in input_keys}

    # Se dopo tutto ciò output_data è ancora vuoto (es. data ha una sola chiave)
    # significa che l'input e l'output coincidono o sono malformati
    if not output_data and input_data:
        output_data = {"warning": "No distinct output field found in synthetic sample"}
