...
class PromptOptimizer:
    ...
    def _run_meta_prompt_backend(self, initial_flag: bool = True) -> Dict:
        ...
        try:
            # ------------------------------------------------------------------
            # ★★★ FIX: INIZIALIZZAZIONE METRICS MANAGER ★★★
            # Senza questo, MetricsManager._output_fields è None e restituisce stringhe vuote.
            # ------------------------------------------------------------------
            import ast
            
            # Gestiamo il caso in cui output_fields sia una stringa "['a', 'b']" o una lista
            if isinstance(self.config.output_fields, str):
                fields_to_configure = ast.literal_eval(self.config.output_fields)
            else:
                fields_to_configure = self.config.output_fields
            
            # Configuriamo il manager statico
            MetricsManager.configure(fields_to_configure)
            print(f"✓ MetricsManager configured manually for Meta Backend. Fields for configurations: {fields_to_configure}")
            # ------------------------------------------------------------------
            ...
            print("🔧 Evaluating initial prompt: ", self.config.task)
            ...
            optimized_prompt = self._call_llm_api_directly(meta_prompt, "gemma3:27b")
        ...
        try:
            ...
            for i, sample in enumerate(all_data):
                try:
                    ...
                    prediction_text = self._call_ollama_api(full_test_prompt, "gemma3:27b")
                    ...
                    print(f"Data before evaluation. Sample: {sample}. Prediction: {prediction}. Prompt: {prompt}")
    ...
    def _call_ollama_api(self, prompt: str, model: str = "") -> str:
        """
        Call Ollama API directly using OpenAI compatible client.
        """
        if model == "":
            model = self.config.config_model_name
        
        # Pulizia del nome modello per Ollama
        model = model.replace("ollama/", "")

        from openai import OpenAI
        
        # Per Ollama Cloud/Remote, l'API Base deve finire con /v1 se usi il client OpenAI
        base_url = self.config.config_model_api_base
        if base_url and not base_url.endswith('/v1'):
            base_url = base_url.rstrip('/') + '/v1'

        client = OpenAI(
            api_key=self.config.config_model_api_key if self.config.config_model_api_key else "ollama",
            base_url=base_url
        )
        ...
        try:
            ...
            # Ollama è solitamente self-hosted/flat rate, ma manteniamo la logica di costo
            # come 0 o come stima simbolica
            self.llm_cost += 0.0 

            # Applichiamo la pulizia per estrarre il contenuto (rimuovere markdown ecc.)
    ...
    def _call_openai_api(self, prompt: str, model: str = "") -> str:
        ...
        # --- NUOVA AGGIUNTA: Pulizia del prefisso per il client OpenAI ---
        # Il client ufficiale 'openai' non vuole prefissi come 'openai/' o 'ollama/'
        ...
        try:
            ...
            if model != "o3" and model != "gemma3:27b":
                return self._clean_llm_response(response_text)
            else:
    ...
    def get_final_eval_metrics(self):
        ...
        print("Task definita per la metrica di valutazione: ", self.config.task_type)
    ...
    def _validate_synthetic_data(self, data: Dict, task: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            response = self._call_ollama_api(prompt, "gemma3:27b")
