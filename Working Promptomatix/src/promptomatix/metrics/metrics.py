...
class MetricsManager:
    ...
    def _get_output_value(item: Any) -> str:
        ...
        values = []
        for field in MetricsManager._output_fields:
            # Tenta come dizionario, poi come oggetto
            val = ""
    ...
    def get_final_eval_metrics(task_type: str) -> Callable:
        ...
        print(f"Returning {task_type} function. Function name: {metrics_map.get(task_type, MetricsManager._qa_metrics_final_eval).__name__}")
    ...
    def _information_extraction_metrics_final_eval(example: Any, pred: Any, instructions: Any = None, trace: Any = None) -> float:
        """
        Robust evaluation for information extraction.
        Handles both Dictionary and Object inputs correctly.
        """
        try:
            gold_info = MetricsManager._get_output_value(example)
            pred_info = MetricsManager._get_output_value(pred)
            print(f"Gold info: {gold_info}. Pred info: {pred_info}.")
            
            # Convert to sets of key-value pairs
            gold_items = set(re.findall(r'"([^"]+)"\s*:\s*"([^"]+)"', gold_info))
            pred_items = set(re.findall(r'"([^"]+)"\s*:\s*"([^"]+)"', pred_info))
            print(f"Gold items: {gold_items}. Pred items: {pred_items}.")
            ...
            print(f"CRITICAL ERROR in metrics: {str(e)}")
            import traceback
            traceback.print_exc()
            return 0.0
