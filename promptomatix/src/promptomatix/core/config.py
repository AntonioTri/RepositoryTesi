...
class Config:
    ...
    def __init__(self, **kwargs):
        ...
        if self.huggingface_dataset_name is None and self.raw_input is not None:
            print("Populate config")
            self._populate_config()
        elif self.huggingface_dataset_name is not None:
            print("Populate config hugging")
    ...
    def _setup_model_config(self):
        ...
        if not self.model_name:
            if self.model_provider is None:
                self.model_provider = ModelProvider.LOCAL
                # self.model_provider = ModelProvider.TOGETHERAI
            self.model_name = PROVIDER_CONFIGS[ModelProvider.LOCAL]['default_model']
        ...
        if not self.config_model_name:
            if self.config_model_provider is None:
                self.config_model_provider = ModelProvider.LOCAL
                # self.config_model_provider = ModelProvider.TOGETHERAI
            self.config_model_name = PROVIDER_CONFIGS_FOR_CONFIG_MODEL[ModelProvider.LOCAL]['default_model']
        ...
        if isinstance(self.config_model_provider, str):
            ...
            if not self.config_model_provider:
                self.config_model_provider = PROVIDER_CONFIGS_FOR_CONFIG_MODEL[ModelProvider.LOCAL]['default_model']
    ...
    def _process_human_feedback(self, tmp_lm) -> str:
        ...
        response = tmp_lm(prompt)[0]
        print(f"Risposta dal modello lm temporaneo nella process human feedback: {response}")
