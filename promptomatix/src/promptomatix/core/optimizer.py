...
class PromptOptimizer:
    ...
    def generate_synthetic_data(self) -> List[Dict]:
        ...
        try:
            ...
            max_samples_per_batch = min(5, max(1, int(8000 / no_of_toks_in_sample_data)))
            ...
            with dspy.settings.context():
                try:
                    ...
                    while remaining_samples > 0:
                        print(f"⏳Sample iteration {iteration_num}")
    ...
    def clean_llm_response(self, response: str) -> str:
        """
        Pulisce la risposta dell'LLM rimuovendo delimitatori, 'SCHEMA' e tag XML.

        Args:
            response (str): Risposta grezza del modello, es. ```SCHEMA\n<instructions>...</instructions>\n```

        Returns:
            str: Testo pulito, es. "Tell me a cute little fairy tale."
        """
        if not response:
            return ""

        # 1. Rimuovi delimitatori Markdown (```SCHEMA ... ```)
        cleaned = re.sub(r"```[a-zA-Z]*", "", response)
        
        # 2. Rimuovi la parola "SCHEMA" e newline inutili
        cleaned = re.sub(r"\bSCHEMA\b", "", cleaned, flags=re.IGNORECASE)
        
        # 3. Rimuovi virgolette iniziali/finali e spazi
        cleaned = cleaned.strip().strip('"').strip("'")

        # 4. Se ci sono tag XML <instructions>...</instructions>, estrai solo il contenuto
        match = re.search(r"<instructions>(.*?)</instructions>", cleaned, flags=re.DOTALL | re.IGNORECASE)
        if match:
            cleaned = match.group(1).strip()

        # 5. Normalizza whitespace
    ...
    def _run_meta_prompt_backend(self, initial_flag: bool = True) -> Dict:
        ...
        try:
            ...
            print(f"✅ Meta-promt generated")
            
            # Get optimized prompt from LLM using direct API calls
            optimized_prompt = self._call_llm_api_directly(meta_prompt, "ollama/gemma3")
            print(f"✅ Prompt optimazed correctly. Optimized prompt: {optimized_prompt}")
    ...
    def _evaluate_prompt_meta_backend(self, prompt: str) -> float:
        ...
        try:
            ...
            print(f"Evaluation data: {all_data}")
    ...
    def _call_llm_api_directly(self, prompt: str, model: str = "") -> str:
        ...
        try:
            ...
            provider = getattr(self.config, 'config_model_provider', 'local')
            ...
            if provider.lower() == 'openai':
                ...
                return self._call_local_ollama_api(prompt)
            else:
    ...
    def _call_local_ollama_api(self, prompt: str, model: str = "") -> str:
        """
        Call local Ollama API (http://127.0.0.1:11434/api/generate).
        """
        try:
            if model == "":
                model = (model or self.config.config_model_name or "gemma3").replace("ollama/", "")

            api_url = (self.config.config_model_api_base or "http://127.0.0.1:11434").rstrip("/")
            ...
            return self.clean_llm_response(text)

        
        except Exception as e:
            self.logger.error(f"Error calling local Ollama API: {str(e)}")
            raise
