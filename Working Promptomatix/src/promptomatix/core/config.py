...
def setup_config_logger():
    ...
    print(f"Config LLM interaction logger initialized. Log file: {log_file}")
...
class Config:
    ...
    def _populate_config_from_huggingface(self):
        ...
        print(f"Prepared datasets - Train: {len(self.train_data)}, Valid: {len(self.valid_data)}, Valid Full: {len(self.valid_data_full)}")
        ...
        print(f"Task description extracted: {self.task_description[:100]}...")
        ...
        if self.tools:
            print(f"Tools extracted for {self.task_type} task")
        ...
        print(f"Selected DSPy module: {self.dspy_module.__name__}")
        ...
        print(f"Synthetic data size: {self.synthetic_data_size}")
    ...
    def _populate_config(self):
        ...
        self.task_description = self._extract_task_description(tmp_lm)
        print(f"Task description extracted: {self.task_description}...")

        self.sample_data = self._extract_sample_data(tmp_lm)
        print(f"Sample data extracted: {self.sample_data}...")
        ...
        if not self.train_data and not self.valid_data:
            ...
            print("Using user-provided data, skipping synthetic data generation")
    ...
    def _setup_model_config(self):
        ...
        PROVIDER_CONFIGS = {
            ...
            ModelProvider.OLLAMA: {
                ...
                'default_model': 'openai/gemma3:27b'
            }
        }
        ...
        if not self.model_name:
            if self.model_provider is None:
                self.model_provider = ModelProvider.OLLAMA
                # self.model_provider = ModelProvider.TOGETHERAI
            self.model_name = PROVIDER_CONFIGS[ModelProvider.OLLAMA]['default_model']
        ...
        if not self.config_model_name:
            if self.config_model_provider is None:
                self.config_model_provider = ModelProvider.OLLAMA
                # self.config_model_provider = ModelProvider.TOGETHERAI
            self.config_model_name = PROVIDER_CONFIGS_FOR_CONFIG_MODEL[ModelProvider.OLLAMA]['default_model']
        ...
        if isinstance(self.config_model_provider, str):
            ...
            if not self.config_model_provider:
                self.config_model_provider = PROVIDER_CONFIGS_FOR_CONFIG_MODEL[ModelProvider.OLLAMA]['default_model']
        ...
        if config_provider_config['env_key']:
            if self.config_model_api_key is None:
                print("Enviroment key: ", config_provider_config['env_key'])
                try:
        ...
        try:
            print("Trying to make a temp lm in setup model configs...")
            tmp_lm = dspy.LM(
            ...
            print(f"Successfully initialized {self.config_model_provider.value} model: {self.config_model_name}")
            print("Successo!")
    ...
    def _process_human_feedback(self, tmp_lm) -> str:
        ...
        print("Human feedback processed and simplified")
        return response
    ...
    def _load_huggingface_datasets(self) -> tuple[Dataset, Dataset]:
        ...
        for option in options:
            try:
                ...
                print(f"Successfully loaded dataset: {self.huggingface_dataset_name}")
                break
            except Exception as e:
    ...
    def _load_user_provided_data(self):
        ...
        if self.train_data or self.valid_data:
            print("Using directly provided train_data/valid_data")
            return
        
        # If local data paths are provided, load from files
        if self.local_train_data_path:
            print(f"Loading training data from: {self.local_train_data_path}")
            ...
            if self.train_data_size and len(train_df) > self.train_data_size:
                ...
                print(f"Restricted training data to {self.train_data_size} samples")
            ...
            if self.local_test_data_path:
                print(f"Loading test data from: {self.local_test_data_path}")
                test_df = pd.read_csv(self.local_test_data_path)
                if self.valid_data_size and len(test_df) > self.valid_data_size:
                    test_df = test_df.head(self.valid_data_size)
                    print(f"Restricted validation data to {self.valid_data_size} samples")
                
                self.valid_data = test_df.to_dict('records') 
                print(f"Loaded {len(self.valid_data)} validation samples")
