...
from promptomatix.lm_manager import LMManager
...
def process_input(**kwargs) -> Dict:
    ...
    try:
        ...
        # Initialize the LM (local or remote)
        print(getattr(config, "model_provider", "local"))
        if getattr(config, "model_provider", "local").value.lower() == "local":
            
            print("🧠 Using local Ollama model via LocalOllama...")
            from promptomatix.lm_manager import LocalOllama
            
            lm = LocalOllama(
                model=config.model_name or "gemma3",
                api_base=getattr(config, "model_api_base", "http://127.0.0.1:11434/api/generate"),
            ...
            print(f"🌐 Using remote provider: {config.model_provider}")
            lm = LMManager.get_lm(
                provider=config.model_provider or "openai",
                model_name=config.model_name or "gpt-4o",
                api_key=getattr(config, "model_api_key", None),
                api_base=getattr(config, "model_api_base", None),
                temperature=getattr(config, "temperature", 0.7),
                max_tokens=getattr(config, "max_tokens", 4000)
            )

        LMManager.configure_dspy(lm)
        ...
        if 'error' in result:
            print(f"❌ Optimization failed in module main: {result['error']}")
            return result
        ...
        print(f"❌ Optimization failed in main exception: {error_msg}")
...
def display_fancy_result(result: Dict) -> None:
    ...
    if 'error' in result:
        print(f"\n{Fore.RED}❌ Optimization Failed in main display fancy results{Style.RESET_ALL}")
